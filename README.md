...
### 5. Machine Learning-Driven Calibration Training
Although the analytical Taylor series expansion provides a local inverse near zero, its accuracy degrades at large signal amplitudes. To achieve optimal performance across the full dynamic range $[-1.0, 1.0]$ (which translates to the digital integer space $[0, 4095]$), we utilize a Machine Learning approach to fit a global polynomial correction model.
...
1.  **Signal Distortion Simulation**: A Python host application [host_app.py](file:///Users/dakshdesai/Codes/FPGA/adc_calibration_ml/host_app.py) that simulates high-frequency analog input signals, injects mathematical non-linear harmonic distortions to simulate physical ADC limitations, streams the distorted codes to the microcontroller, and reads back the calibrated results.
2.  **Interface Bridge**: An RP2040 microcontroller running [rp2040_bus.py](file:///Users/dakshdesai/Codes/FPGA/adc_calibration_ml/rp2040_bus.py) that acts as the communication link. To prevent data transmission bottlenecks and maintain high sampling rates, it interfaces with the FPGA over a 4-bit bidirectional parallel bus using direct memory-mapped access to the RP2040 SIO registers (bypassing slow hardware abstraction layers).
3.  **Sequential FPGA Polynomial Engine**: A Verilog processor synthesized for the SLG47910 FPGA (using [top.v](file:///Users/dakshdesai/Codes/FPGA/adc_calibration_ml/rtl/top.v)).
    *   [parallel_rx.v](file:///Users/dakshdesai/Codes/FPGA/adc_calibration_ml/rtl/parallel_rx.v) deserializes raw 12-bit ADC data received in 3 sequential nibbles.
    *   [adc_calibrator.v](file:///Users/dakshdesai/Codes/FPGA/adc_calibration_ml/rtl/adc_calibrator.v) implements a sequential 12-state FSM that performs the polynomial calculations. It uses a single shared signed multiplier block to calculate the coefficients sequentially, reducing the physical microcell count and logic density.
    *   [parallel_tx.v](file:///Users/dakshdesai/Codes/FPGA/adc_calibration_ml/rtl/parallel_tx.v) streams the calibrated 12-bit values back to the RP2040 over the 4-bit parallel bus using strobe synchronization.
